...
class Tweet:
    ...
    def jsonstring(self):
        text = "{\"id\": " + str(self.id)
        text += ", \"datetime\": \"" + str(self.datetime) + "\""
        text += ", \"original_text\": " + json.dumps(self.original_text)
        text += ", \"cleaned_text\": " + json.dumps(self.cleaned_text)
        text += ", \"author\": " + json.dumps(self.author)
        text += ", \"entities\": " + json.dumps(self.entities)
        text += ", \"retweet\": \"" + str(self.retweet) + "\""
        text += ", \"retweet_author\": " + json.dumps(self.retweet_author)
        text += ", \"quote\": \"" + str(self.quote) + "\""
        text += ", \"quoted_text\": " + json.dumps(self.quoted_text) + "}"
        return text
    ...
    def clean_text(self, name_dictionary = {}):
        ...
        for hashtag in self.entities['hashtags']:
            cleaned_text = cleaned_text[:cleaned_text.index("#"+hashtag["text"])]+" "+self.wordify(hashtag["text"])+" "+cleaned_text[cleaned_text.index("#"+hashtag["text"])+len("#"+hashtag["text"]):]
        #Remove links
        for url in self.entities['urls']:
            if url["url"] in cleaned_text:
                cleaned_text = cleaned_text[:cleaned_text.index(url["url"])]+"."+cleaned_text[cleaned_text.index(url["url"])+len(url["url"]):]
        #Remove media
        for media in self.entities['media']:
            if media["url"] in cleaned_text:
                cleaned_text = cleaned_text[:cleaned_text.index(media["url"])]+"."+cleaned_text[cleaned_text.index(media["url"])+len(media["url"]):]
        #Remove RT
        if self.retweet:
        ...
        for user_mention in self.entities['user_mentions']:
            if user_mention["screen_name"].lower() in cleaned_text.lower() :
                ...
                cleaned_text = cleaned_text[:cleaned_text.lower().index("@"+user_mention["screen_name"].lower())]+" "+name_to_use+", "+cleaned_text[cleaned_text.lower().index("@"+user_mention["screen_name"].lower())+len("@"+user_mention["screen_name"]):]
    ...
    def __init__(self, tweepy_tweet = None, name_dictionary = {}):
        if tweepy_tweet :
            ...
            if 'retweeted_status' in tweepy_tweet.keys():
                self.retweet = True
                self.original_text = re.sub('https://t.co/[A-Za-z0-9]+$','',tweepy_tweet['retweeted_status']['full_text'])
                self.datetime = datetime.datetime.strptime(tweepy_tweet['retweeted_status']['created_at'], '%a %b %d %H:%M:%S +0000 %Y')
                ...
                self.entities['media'] = tweepy_tweet['retweeted_status']['entities']['media'] if 'media' in tweepy_tweet['retweeted_status']['entities'].keys() else []
                ...
                self.entities['media'] = tweepy_tweet['entities']['media'] if 'media' in tweepy_tweet['entities'].keys() else []
            self.quote = tweepy_tweet['is_quote_status']
            self.quoted_text = tweepy_tweet['quoted_status']['full_text'] if 'quoted_status' in tweepy_tweet.keys() else ''        
